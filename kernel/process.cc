    ...
    Resource(ResourceType::PROCESS), name(name), resources(resources_)
{
    ...
    uid = 0;            // default set to 0 as root
    gid = 0;            // default set to 0 as root
}
...
long Process::execv(const char* fileName, SimpleQueue<const char*> *args, long argc) {
    ...
    // copy the args to top of the stack
    char **argv = new char *[argc];
    int index = 0;
    while (!args->isEmpty()) {
        const char *arg = args->removeHead();
        uint32_t len = K::strlen(arg) + 1;
        char *dest = (char *)(userESP - len);
        memcpy(dest, arg, len);
        argv[index++] = dest;
        userESP -= len;
        delete arg;
    ...
    for (int i = argc - 1; i >= 0; i--) {
        userESP -= 4;
        *(uint32_t *) userESP = (uint32_t) argv[i];
    }
    userESP -= 4;
    *(uint32_t *) userESP = userESP + 4;
    userESP -= 4;
    *(uint32_t *) userESP = argc;

    delete [] argv;

    // Debug::printf("userESP: 0x%8x\n", userESP);
